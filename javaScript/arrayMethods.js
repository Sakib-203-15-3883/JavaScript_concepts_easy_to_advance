...
//example 1: we want to add 2(value) with each array element using addByTwo function.
//here , in example 1 , we need to invoke the function (addByTwo) for eacj array element . we can,t do same thing again and again .
...
//example 2:we want to multiply 2 with each array element using multiplyByTwo function  . here we use for loop for reduce code repetation 
...
//example 3: we want to divided  each array element by 2 using dividedByTwo function  . here we use forEach() method . forEach() method takes a function as input which known as callback function.it,s not work with empty array 

function dividedByTwo(number) {
    console.log(number / 2);
}
numbers.forEach(dividedByTwo) // forEach method pass  value for each array element .




//example 4:we have an array called users  and inside array we have 4 objects with the property of (firstName and age). now we need to print firstName and age from each object without using forEach method.
...
//example 5:we have an array called users1  and inside array we have 4 object with the property of (firstName and age). now we need to print firstName and age from each object  using forEach method.
...
const numberss = [2,4,6,8,10];
    ...
    //example 1: we want to add 2(value) with each array element using map() method . map() method will stored performed value(performed by function) in a new array.
...
console. log(newNumber);
    ...
    //example 5:we have an array called person  and inside array we have 3 object with the property of (firstName and age). now we need to print firstName and age  from each object  using map() method. map() method store all the full name in a new array
...
function odd (number){
  ...
  const listOfEvenNumber = numbersForFilter.filter(even);
  ...
  console. log( sumOfAllArrayElement);
   ...
   // example 15 : E-commerce related  , suppose a user select 3 different product . now you need to calculate the sum of those 3 product using reduce() method.
...
const totalAmount = userCard.reduce((totalPrice,currentProduct) => {return totalPrice+currentProduct.price},0);
...
// that,s why we need to clone the array . we can do it using slice()
...
const lowToHigh =usersForSortExplain.slice(0).sort((a,b) => a.price-b.price);
...
console. log(animal.find((string)=>{return string.length===3}));
...
const priceRange = usersForFindExplain.find((product) => {return product.productId ===3 })
...
const ans = numbersForEveryMethod.every((number) => {return number%2===0});
...
const rangeOfPrice = usersForEveryExplain.every((product) => { return product.price<15000});
...
const ans1 = numbersForSomeMethod.some((number) => { return  number>6});
...
const rangeOfPrice1 = usersForSomeExplain.some((product) => { return product.price<15000});
...
//The push() method removes the last element from an array:
