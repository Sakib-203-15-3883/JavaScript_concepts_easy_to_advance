...
//7. Use const wherever possible followed by let, Use var as little as possible(only if you have to). It helps avoid error.8. Initialising variables at the top is good idea, helps shrinks TDZ to zero.


// declaring and  Initialising variables at the top is good idea for avoiding temporal deadzone for let and const keyword .TDZ refers to the time between the creation of a variable and its initialization with a value, during which time the variable is in an "uninitialized" state and cannot be accessed or assigned a value without throwing a ReferenceError.
